...
class CampaignSender
{
    ...
    public function stop(Campaign $campaign){
        $instances = $this->em->getRepository('AdenaMailBundle:MailEngineInstance')->findBy([
            'campaign' => $campaign
        ]);
    ...
    private function _doSend(Campaign $campaign, $successStatus, $errorStatus, $logName)
    {
        ...
        // new queueOperator - array de queue
        // while queue
        ...
        // mailEngine (only ONE message, no queues)
        // mailEngine error management (try catch)
        // if no senders set $errorStatus
